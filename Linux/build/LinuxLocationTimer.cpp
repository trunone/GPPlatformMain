...
LinuxLocationTimer::LinuxLocationTimer(LocationManager* manager)
    : m_Manager(manager)
{
...
void *LinuxLocationTimer::TimerProc(void *param)
{
    LinuxLocationTimer *timer = (LinuxLocationTimer *)param;
    ...
    {
        next_time.tv_sec += (next_time.tv_nsec + LocationModule::TIME_UNIT * 1000000) / 1000000000;
        next_time.tv_nsec = (next_time.tv_nsec + LocationModule::TIME_UNIT * 1000000) % 1000000000;
...
void LinuxLocationTimer::Start(void)
{
    int error;
...
void LinuxLocationTimer::Stop(void)
{
    int error=0;
...
LinuxLocationTimer::~LinuxLocationTimer()
{
    this->Stop();
    this->m_Manager = NULL;
}

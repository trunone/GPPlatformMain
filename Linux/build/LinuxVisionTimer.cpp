...
LinuxVisionTimer::LinuxVisionTimer(VisionManager* manager)
    : m_Manager(manager)
{
...
void *LinuxVisionTimer::TimerProc(void *param)
{
    ...
    {
        next_time.tv_sec += (next_time.tv_nsec + VisionModule::TIME_UNIT * 1000000) / 1000000000;
        next_time.tv_nsec = (next_time.tv_nsec + VisionModule::TIME_UNIT * 1000000) % 1000000000;
...
void LinuxVisionTimer::Stop(void)
{
    int error=0;
...
LinuxVisionTimer::~LinuxVisionTimer()
{
    this->Stop();
    this->m_Manager = NULL;
}
